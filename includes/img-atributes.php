...
/*******************************************************/
    ...
    /*
        Escreve os atributos das imagens
            ...
            Escrever isso tudo na tag img ajuda na velocidade de carregamento e no SEO
            Retorna toda a tag img toda escrita

        Sobre os parametros:
        $id
            Se for thumbnail vem o id do post
            se for acf vem o id da img
        ...
        $size_to_serve
            ...
            Para resolver o seguinte problema:
                Se for um banner/hero precisamos servir em um tamanho grande.
                Se for um thumb em uma lista de posts podemos servir em um tamanho menor.
...
function pipe_get_img_atributes($id, $is_thumb, $size_to_serve, $class)
{
    ...
    // buscas os atributos da img (texto alternativo, title, ...)
    $attachment = get_post($img_id);

    // Busca o srcset da img
    $img_srcset = wp_get_attachment_image_srcset($img_id,);

    //Seta a altura e largura que queremos servir a imagem
    $new_width = pipe_img_new_size($attachment, $size_to_serve)['width'];
    $new_height = pipe_img_new_size($attachment, $size_to_serve)['height'];

    //seta o texto alternativo
    $alt_text = get_post_meta($attachment->ID, '_wp_attachment_image_alt', true);

    //Seta as sugestões de tamanhos para cada largura de tela
    $size_small = pipe_img_srcset($img_srcset)['small'];
    $size_medium = pipe_img_srcset($img_srcset)['medium'];
    $size_large = pipe_img_srcset($img_srcset)['large'];
    ...
    $image = '
        ...
        sizes="
            (max-width: 576px) ' . $size_small . 'vw,
            (max-width: 992px) ' . $size_medium . 'vw,
            (max-width: 1300) ' . $size_large . 'vw,
            ' . $size_medium . 'vw" >
    ';
...
function pipe_img_srcset($img_srcset)
{
    ...
    /*
        ...
        Com o array de srcset temos todos os tamanhos para poder sugerir 03
    */
